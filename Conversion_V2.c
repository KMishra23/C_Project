...
void opentiff(const char *filename, struct BMP_Header *bmphead) {
   ...
   printf("Offset to IFD: %d \n", TIFHEAD.IFDOffset);
    ...
    fseek(fp,TIFHEAD.IFDOffset,SEEK_SET);
    ...
    {
        //find size of IFD by finding number of IFD entries and multiply it with size of each IFD 
        fseek(fp, TIFHEAD.IFDOffset, SEEK_SET);
        int read_number_of_IFDEntries[2];
        for(i = 0; i<2 && (c=getc(fp)) != EOF; i++)
            read_number_of_IFDEntries[i] = c;
        DWORD number_of_IFDEntries;
        if(endian_identifier==0)
            number_of_IFDEntries = getDwordFromBytes_littleEndian(read_number_of_IFDEntries,2);
        else
            number_of_IFDEntries = getDwordFromBytes_bigEndian(read_number_of_IFDEntries,2);
